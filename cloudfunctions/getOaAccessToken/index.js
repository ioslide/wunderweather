...
exports.main = async event => {
  ...
  let accessToken = {}
  try {
    accessToken = (await db.collection('oa-subscribe-accessToken').doc('ACCESS_TOKEN').get()).data
    ...
    if (accessToken.time > overtime) {
      ...
      if (result.access_token != null) {
        ...
        await db.collection('oa-subscribe-accessToken').doc('ACCESS_TOKEN').update({
          time: db.serverDate({
    ...
    if (result.access_token != null) {
      const { access_token, expires_in } = result
      try {
        let resAdd = await db.collection('oa-subscribe-accessToken').doc('ACCESS_TOKEN').update({
              data: {
                time: db.serverDate({
                  offset: expires_in * 1000
                }),
