	...
	var Helpers = {
		...
		getScaledMatrix: function getScaledMatrix(shape, v, direct) {
			...
			if (direct === 'x') {
				...
				scaledMatrix = Matrix$1.transform(_matrix, [
					['t', x, y],
					['s', 1, 100],
					['t', -x, -y]
				]);
				...
				scaledMatrix = Matrix$1.transform(_matrix2, [
					['t', x, y],
					['s', 100, 100],
					['t', -x, -y]
				]);
				...
				if (version[0] > 1 || 1 === version[0] && version[1] > 9 || 1 === version[0] && 9 === version[1] && version[2] >= 91) {
					...
					query.select('.f2-canvas')
						...
						.exec(res => {
							...
							console.log('config',t.data.config)
							const chart = this.data.onInit(lib, t.data.config);
							if (chart) {
								this.chart = chart;
