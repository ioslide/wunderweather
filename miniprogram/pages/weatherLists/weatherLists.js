...
create(store, {
  ...
  onLoad: function () {
    ...
    t.setData({
      weatherImageLists:headImageData.weatherImageLists
    })
  },
  onShow() {
    const t = this
  ...
  showModal(e) {
    var t = this
    t.setData({
      // modalImage: wx.getStorageSync('weatherImageLists')[e.currentTarget.dataset.order],
  ...
  downloadImageModal: function () {
    ...
    wx.showModal({
      title: '是否下载图片',
      content: t.data.weatherImageLists[t.data.currDay].title,
      success(res) {
        if (res.confirm) {
          t.downloadCur()
        } else if (res.cancel) {}
  ...
  downloadCur: function () {
    ...
    wx.authorize({
      ...
      success: function success() {
        t.downloading = true;
        let imgUrl = t.data.weatherImageLists[t.data.currDay].url.p4
        wx.downloadFile({
          url: imgUrl,
